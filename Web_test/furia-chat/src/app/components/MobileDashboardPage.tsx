...
const MobileDashboardPage: React.FC<MobileDashboardPageProps> = ({ onLogout, toggleSettingsMenu, isSettingsOpen, isActive }) => {
    const router = useRouter()

    return (
        <div className="flex flex-col h-screen bg-gray-50 items-center p-6"> {/* Layout flex column e centralizado */}
            <h2 className="text-xl font-semibold mb-2 md:text-2xl text-center">FURIA x NAVI</h2>
            <p className="text-sm text-gray-500 md:text-base mb-4 text-center">
                18:00 - Latest news headline...
            </p>
            <div className="flex flex-col gap-4 w-full max-w-sm"> {/* Container para os botões */}
                <button
            ...
            <nav className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 py-3 flex justify-around items-center">
                {[
                    ...
                    <div key={base} className="relative">
                        ...
                        >
                            <Image
                                src={`/icons/${base}${isActive(route || "") ? "_hover" : ""}.${ext}`}
                                alt={alt}
                                width={28}
                                height={28}
                            />
